...
from voice.manager import VoiceChatManager
...
class UserbotClient:
    def __init__(self, user_id: int, session_string: str):
        ...
        self.voice_chat: VoiceChatManager | None = None
    ...
    async def start(self) -> None:
        ...
        self._running = True
        try:
            self.voice_chat = VoiceChatManager(self.user_id, self.client)
            setattr(self.client, "_voice_chat_manager", self.voice_chat)
            await self.voice_chat.start()
        except Exception:
            self.voice_chat = None
            setattr(self.client, "_voice_chat_manager", None)
            logger.exception(
                "Voice-chat manager could not start for user %s; "
                "the message/AI plugins will continue.",
                self.user_id,
            )
        if (
