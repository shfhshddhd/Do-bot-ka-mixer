...
# ── Per-hosted-account VC control and relay state ──────────────────────────────

async def get_voice_control(user_id: int) -> dict | None:
    """Return the hosted account's registered control/source groups."""
    user = await get_user(user_id)
    if not user or user.get("voice_control_chat_id") is None:
        return None
    source_chat_id = user.get("voice_source_chat_id", user["voice_control_chat_id"])
    return {
        "control_chat_id": int(user["voice_control_chat_id"]),
        "source_chat_id": int(source_chat_id),
    }


async def get_voice_owner(control_chat_id: int) -> int | None:
    """Resolve one registered control group to its active host owner."""
    document = await get_db().users.find_one(
        {"voice_control_chat_id": int(control_chat_id), "active": True},
        {"_id": 0, "user_id": 1},
    )
    return int(document["user_id"]) if document and document.get("user_id") is not None else None


async def set_voice_control(
    user_id: int,
    control_chat_id: int,
    ...
    await upsert_user(
        user_id,
        {
            "voice_control_chat_id": int(control_chat_id),
            "voice_source_chat_id": source_chat_id,
        },
    )


async def clear_voice_control(user_id: int) -> None:
    await get_db().users.update_one(
        {"user_id": user_id},
        {"$unset": {"voice_control_chat_id": "", "voice_source_chat_id": "", "voice_sessions": ""}},
    )


async def get_voice_sessions(user_id: int) -> list[dict]:
    user = await get_user(user_id)
    sessions = user.get("voice_sessions", []) if user else []
    return sessions if isinstance(sessions, list) else []


async def save_voice_sessions(user_id: int, sessions: list[dict]) -> None:
    await upsert_user(user_id, {"voice_sessions": sessions})


async def clear_voice_sessions(user_id: int) -> None:
    await get_db().users.update_one(
        {"user_id": user_id},
        {"$unset": {"voice_sessions": ""}},
    )
